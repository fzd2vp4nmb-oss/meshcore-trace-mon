...
# (nessuna logica di continue-past-error qui, quindi anche set -e
# sarebbe innocuo, ma non aggiunge nulla su uno script di due righe:
# omesso per coerenza con lo stile già in uso negli altri script).
#
set -u
set -o pipefail
...
# send_advert.py sotto — resta una scelta deliberata (v.
# docs/CHANGES_log_errori_cron_su_file.md, nota Rev5): un singolo
# advert mancato non è di per sé un evento da tracciare, il prossimo
...
LOCKFILE="$INSTALL_DIR/run/advert.sh.lock"
